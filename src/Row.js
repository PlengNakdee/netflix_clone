...
function Row({ title, fetchUrl, isLargeRow }) {
  ...
  // runs based on a specific condition/variable
  // if[], run once when row loads and don't run again
  // if[fetchUrl], run every time when fetchUrl variable change
  useEffect(() => {
  ...
  return (
    <div className="row">
      ...
      <div className="row__posters">
        {movies.map((movie) => (
          <img
            key={movie.id} //re-render only movie.id, optimization, faster
